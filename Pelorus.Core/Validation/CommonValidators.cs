...
namespace Pelorus.Core.Validation
{
    ...
    {
        private static Regex emailRegex = new Regex(@"^(?("")(""[^""]+?""@)|(([0-9a-zA-Z]((\.(?!\.))|[-!#\$%&'\*\+/=\?\^`\{\}\|~\w])*)(?<=[0-9a-zA-Z])@))(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-zA-Z][-\w]*[0-9a-zA-Z]*\.)+[0-9a-zA-Z]{2,24}))$");
        private static Regex phoneNumberRegex = new Regex(@"[\(]{0,1}[0-9]{3}[\s]{0,}[-\.\*\)]{0,1}[\s]{0,}[0-9]{3}[\s]{0,}[-\.\*\)]{0,1}[\s]{0,}[0-9]{4}");
        private static Regex zipCodeRegex = new Regex(@"[0-9]{5}(-[0-9]{4}){0,1}");
